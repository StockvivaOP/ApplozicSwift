...
enum Options: String, Localizable {
    ...
    func value(
        ...
        case .info:
            let title = ALKConfiguration.delegateSystemInfoRequestDelegate?.getSystemTextLocalizable(key: "chat_common_info") ?? localizedString(forKey: "Info", withDefaultValue: SystemMessage.GroupDetails.Info, fileName: localizationFileName)
            return UIAlertAction(title: title, style: .default, handler: { (_) in
...
class ALKCreateGroupViewModel: Localizable {
    ...
    private func getCurrentUserInfo() -> GroupMemberInfo {
        ...
        let name = ALKConfiguration.delegateSystemInfoRequestDelegate?.getSystemTextLocalizable(key: "chat_common_you") ??  localizedString(forKey: "You", withDefaultValue: SystemMessage.LabelName.You, fileName: localizationFileName)
        return GroupMemberInfo(
